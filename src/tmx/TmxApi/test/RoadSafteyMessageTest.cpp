...
TEST(RoadSafetyMessageTest, EncodeRoadSafetyMessageXML)
{
	...
	std::string hexString = "0021430700e51f418080222020218181431f9fa0e6f7800b400fe0e0e0200000a66e7b951ea6e2ac88c306c1f5f96fdd9d057c3e5044e5a7b65e40299b9ee547a9b8ab2230c0";
	EXPECT_EQ(hexString, rsmENC.get_payload_str()); 
}
...
TEST(RoadSafetyMessageTest, EncodeRoadSafetyMessageLaneClosure) {
	...
	*/
	...
	auto message = std::shared_ptr<RoadSafetyMessage_t>(
		static_cast<RoadSafetyMessage_t*>(calloc(1, sizeof(RoadSafetyMessage_t))),
		[](RoadSafetyMessage_t *p) {
			j2735::j2735_destroy<RsmTraits>(p);
		} 
	);

	// Road Authority OID
	std::vector<uint32_t> oid = {1,0,15628,4,1,17,1,0};
	message->commonContainer.eventInfo.eventID.operatorID.present=RoadAuthorityID_PR_fullRdAuthID;
	asn_fprint(stdout, &asn_DEF_EventInfo, &message->commonContainer.eventInfo);
	bool failed = OBJECT_IDENTIFIER_set_arcs(&message->commonContainer.eventInfo.eventID.operatorID.choice.fullRdAuthID, oid.data(), oid.size());
		// If operation fails, unit test is no longer valid
	ASSERT_EQ(failed, 0);

	char  my_bytes_id[4] = {(char)1, (char)12, (char)12, (char)10};
	failed = OCTET_STRING_fromBuf(&message->commonContainer.eventInfo.eventID.uniqueID, my_bytes_id, sizeof(my_bytes_id));
	// If operation fails, unit test is no longer valid
	ASSERT_EQ(failed, 0);


	// Event update version
	message->commonContainer.eventInfo.eventUpdate = 12;

	// Is event cancelled
	message->commonContainer.eventInfo.eventCancellation = 0;
	...
	*minute = 0;
	*second = 0;
	message->commonContainer.eventInfo.startDateTime.year = year;
	message->commonContainer.eventInfo.startDateTime.month = month;
	message->commonContainer.eventInfo.startDateTime.day = day;
	message->commonContainer.eventInfo.startDateTime.hour = hour;
	message->commonContainer.eventInfo.startDateTime.minute = minute;
	message->commonContainer.eventInfo.startDateTime.second = second;

	// ITIS cause codes
	message->commonContainer.eventInfo.causeCode = 2;
	...
	*affectedVehCnt = 9217;
	...
	message->commonContainer.eventInfo.affectedVehicles = rsmAffectedVehicles;
	...
	*elev = 2380;
	...
	approachRegion->choice.broadRegion.broadArea.present = BroadRegionArea_PR_polygon;
	...
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node1);
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node2);
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node3);
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node4);
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node5);
	asn_sequence_add(&approachRegion->choice.broadRegion.broadArea.choice.polygon.list.array, node6);
	regionInfoCnt->approachRegion = approachRegion;

	asn_sequence_add(&message->commonContainer.regionInfo.list.array, regionInfoCnt);
	...
	contentCnt->choice.laneClosureContainer.laneStatus = lnStatus;

	contentCnt->choice.laneClosureContainer.closureRegion.referencePoint.lat = 423010836;
	contentCnt->choice.laneClosureContainer.closureRegion.referencePoint.Long = -836990707;
	auto elevClosure = (Common_Elevation_t*) calloc(1, sizeof(Common_Elevation_t));
	*elevClosure = 2380;
	contentCnt->choice.laneClosureContainer.closureRegion.referencePoint.elevation = elevClosure;
	auto refPointType = (ReferencePointType_t*) calloc(1, sizeof(ReferencePointType_t));
	*refPointType = 0;
	contentCnt->choice.laneClosureContainer.closureRegion.referencePointType = refPointType;
	...
	asn_sequence_add(&closureApproachReg->choice.paths, closurePath);
	contentCnt->choice.laneClosureContainer.closureRegion.approachRegion = closureApproachReg;

	asn_sequence_add(&message->content.list.array, contentCnt);

	asn_fprint(stdout, &asn_DEF_RoadSafetyMessage, message.get());

	// Encode RSM 
	tmx::messages::RsmEncodedMessage RsmEncodeMessage;
	tmx::messages::RsmMessage _rsmMessage(message);
