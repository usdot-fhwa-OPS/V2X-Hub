...
namespace unit_test{
    ...
    TEST_F(TelemetryDeserializerTest, desrializeFullPluginTelemetryPayload){
        ...
        auto jsonContainer = TelemetryDeserializer::stringToTree(jsonString);
        auto telemetryList =  TelemetryDeserializer::deserializeFullTelemetryPayload<PluginTelemetry>(jsonContainer);
    ...
    TEST_F(TelemetryDeserializerTest, desrializeFullPluginTelemetryPayloadEmpty){
        ASSERT_THROW(TelemetryDeserializer::deserializeFullTelemetryPayload<PluginTelemetry>(TelemetryDeserializer::stringToTree("")), tmx::TmxException);
    }

    TEST_F(TelemetryDeserializerTest, desrializeFullPluginTelemetryPayloadNotArray){
        ASSERT_THROW(TelemetryDeserializer::deserializeFullTelemetryPayload<PluginTelemetry>(TelemetryDeserializer::stringToTree("{\"payload\":{\"test\": \"0\"}}")), tmx::TmxException);
    }

    TEST_F(TelemetryDeserializerTest, desrializeFullPluginTelemetryPayloadContentEmpty){
        ASSERT_THROW(TelemetryDeserializer::deserializeFullTelemetryPayload<PluginTelemetry>(TelemetryDeserializer::stringToTree("{\"payload\":[{}]}")), tmx::TmxException);
    }
}