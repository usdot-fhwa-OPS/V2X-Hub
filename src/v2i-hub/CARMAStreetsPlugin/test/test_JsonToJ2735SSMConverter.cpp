...
namespace CARMAStreetsPlugin
{
    ...
    TEST_F(test_JsonToJ2735SSMConverter, toJ2735SSM)
    {
        ...
        bool result = converter.parseJsonString(valid_json_str, root);
        EXPECT_TRUE(result);
        auto ssmPtr = tmx::messages::j2735::j2735_create<tmx::messages::SsmTraits>();
        converter.toJ2735SSM(root, ssmPtr);
        EXPECT_EQ(1, ssmPtr->status.list.count);
        EXPECT_EQ(1003, ssmPtr->status.list.array[0]->id.id);
        EXPECT_EQ(54000, ssmPtr->second);
        EXPECT_EQ(345239, *ssmPtr->timeStamp);
        EXPECT_EQ(3, ssmPtr->status.list.array[0]->sigStatus.list.count);
    }
    ...
    TEST_F(test_JsonToJ2735SSMConverter, toJ2735SSM_2)
    {
        ...
        converter.toJ2735SSM(root, ssmPtr);
        EXPECT_EQ(1, ssmPtr->status.list.count);
        EXPECT_EQ(1004, ssmPtr->status.list.array[0]->id.id);
        EXPECT_EQ(54001, ssmPtr->second);
        EXPECT_EQ(345240, *ssmPtr->timeStamp);
        EXPECT_EQ(1, ssmPtr->status.list.array[0]->sigStatus.list.count);
    }
    ...
    TEST_F(test_JsonToJ2735SSMConverter, toJ2735SSM_3)
    {
        ...
        auto ssmPtr = tmx::messages::j2735::j2735_create<tmx::messages::SsmTraits>();
        converter.toJ2735SSM(root, ssmPtr);
        EXPECT_EQ(0, ssmPtr->status.list.array[0]->sigStatus.list.count);
    }
    ...
    TEST_F(test_JsonToJ2735SSMConverter, encodeSSM)
    {
        ...
        EXPECT_EQ(expected_payload_hex, encodedSSM.get_payload_str());
    }

    TEST_F(test_JsonToJ2735SSMConverter, encodeSSMThreeRequestors) {
        std::string valid_json_str = "{\"MessageType\":\"SSM\",\"SignalStatus\":{\"intersectionID\":1003,\"minuteOfYear\":345239,\"msOfMinute\":54000,\"regionalID\":0,\"requestorInfo\":[{\"ETA_Duration\":2000,\"ETA_Minute\":1,\"ETA_Second\":20.001000000000005,\"basicVehicleRole\":16,\"inBoundLaneID\":8,\"msgCount\":2,\"priorityRequestStatus\":4,\"requestID\":5,\"vehicleID\":601},{\"ETA_Duration\":2000,\"ETA_Minute\":1,\"ETA_Second\":22.001000000000005,\"basicVehicleRole\":16,\"inBoundLaneID\":12,\"msgCount\":6,\"priorityRequestStatus\":4,\"requestID\":5,\"vehicleID\":605},{\"ETA_Duration\":2000,\"ETA_Minute\":1,\"ETA_Second\":22.001000000000005,\"basicVehicleRole\":9,\"inBoundLaneID\":12,\"msgCount\":6,\"priorityRequestStatus\":0,\"requestID\":5,\"vehicleID\":610}],\"sequenceNumber\":4,\"updateCount\":4},\"noOfRequest\":3}";
        ...
        converter.encodeSSM(ssmPtr, encodedSSM);
        std::string expected_payload_hex = "001e3b454497d2f0001007d625ca000004b20a0900100000200280fa08b9400000974143200300000400581f41172800001310286240600000800b03e800";
        EXPECT_EQ(expected_payload_hex, encodedSSM.get_payload_str());
    }
}