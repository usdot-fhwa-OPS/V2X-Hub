...
- **psid** is *not* used for the broadcast. The PSID carried by the SPDU is sent instead, so that
  what goes out over the air matches the message that was received. When the two disagree a warning
  is logged, since it means the configuration is stale.
- **signMessages** must be `true`. The payload is already signed, so RSU4.1 connections send
  `Signature=True` and NTCIP 1218 rows must be initialized with `rsuIFMOptions` `0x80`. SPDUs sent to
  an NTCIP 1218 connection with `signMessages` set to `false` are skipped and logged as an error.
