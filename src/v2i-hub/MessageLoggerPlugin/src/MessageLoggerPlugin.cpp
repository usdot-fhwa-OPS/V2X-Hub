...
void MessageLoggerPlugin::HandleBasicSafetyMessage(BsmMessage &msg, routeable_message &routeableMsg) 
{
	...
	try {
		auto bsm = routeableMsg.get_payload_bytes();
		PLOG(logDEBUG) << bsm;

		// Decode BSM
		tmx::messages::BsmEncodedMessage decodedMessage;
		decodedMessage.set_data(bsm);
		auto decodedBsmPtr = decodedMessage.decode_j2735_message();
		...
		PLOG(logDEBUG) << "Decoded BSM: " << decodedBsmPtr;
	}
