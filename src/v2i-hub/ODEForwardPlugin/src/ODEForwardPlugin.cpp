...
void ODEForwardPlugin::sendUDPMessage(routeable_message &routeableMsg, UDPMessageType udpMessageType) const{
	std::string hex = routeableMsg.get_payload_str().c_str();
	_udpMessageForwarder->sendMessage(udpMessageType, hex);
	PLOG(logDEBUG) << "Sent UDP " + _udpMessageForwarder->toString(udpMessageType)+ " Hex: " << hex;
}
